...
Only output the code, no markdown backticks, no explanations.`;
  ...
  try {
    ...
    if (data.usageMetadata) {
      ...
      const reqEl = $('usage-req');
      const inEl = $('usage-in');
      const outEl = $('usage-out');
      const costEl = $('usage-cost');
      const barFill = document.querySelector('.ai-usage-bar-fill');
      
      if (reqEl && inEl && outEl && costEl) {
        let reqs = parseInt(reqEl.textContent) || 0;
        let ins = parseInt(inEl.textContent) || 0;
        let outs = parseInt(outEl.textContent) || 0;
        ...
        // Rough cost estimation: $0.075 per 1M input, $0.30 per 1M output (Flash tier)
        const cost = (ins / 1000000) * 0.075 + (outs / 1000000) * 0.30;
        costEl.textContent = '$' + cost.toFixed(4);
        
        if (barFill) {
          const maxTokens = 100000; // arbitrary max for visual
          const pct = Math.min(100, ((ins + outs) / maxTokens) * 100);
