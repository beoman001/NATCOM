...
static void nc_value_f(const char*name,double v){
    printf("  " CYAN "%s" RESET " = " BOLD CYAN "%g" RESET "\\n",name,v);}
static void nc_value_s(const char*name,const char*v){
    printf("  " CYAN "%s" RESET " = " BOLD WHITE "%s" RESET "\\n",name,v);}
...
class SystemsCodeGen:
    ...
    def visit(self, node, indent="    "):
        ...
        # ── OOP & Functions ─────────────────────────────────────────────────
            ...
            if js:
                self.out_code.append(f"async function {node.name}({args_str}) {{ // :function")
            else:
        ...
        # ── Variable Declarations ───────────────────────────────────────────
            ...
            if js:
                ...
                vtype = "int" if "integer" in node.var_type else "float"
                c.append(f"{indent}{prefix}{node.name} = {node.value}; // :{vtype}")
            else:
        ...
        # ── User Input ──────────────────────────────────────────────────────
            ...
            safe_p = node.prompt.replace('"', '\\"').replace("'", "\\'")
            if js:
                c.append(f"{indent}// [INPUT:number] {node.var_name}")
            ...
            safe_p = node.prompt.replace('"', '\\"').replace("'", "\\'")
            if js:
                c.append(f"{indent}// [INPUT:string] {node.var_name}")
